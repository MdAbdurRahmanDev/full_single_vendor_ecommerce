        ...
        <div class="flex flex-col lg:flex-row gap-12 max-w-7xl mx-auto">
                    ...
                    <form action="#" method="POST" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-2">
                                ...
                                <input type="text" placeholder="MD Abdur Rahman" class="w-full px-5 py-3 rounded-2xl border border-gray-200 focus:ring-1 focus:ring-brand focus:border-brand bg-gray-50/50">
                            </div>
                            <div class="space-y-2">
                                <label class="text-sm font-bold text-gray-700">Phone Number</label>
                                <input type="tel" placeholder="01700 000 000" class="w-full px-5 py-3 rounded-2xl border border-gray-200 focus:ring-1 focus:ring-brand focus:border-brand bg-gray-50/50">
                            </div>
                            <div class="space-y-2 md:col-span-2">
                                <label class="text-sm font-bold text-gray-700">Shipping Address</label>
                                <textarea rows="3" placeholder="House No, Road, Area, Dhaka" class="w-full px-5 py-3 rounded-2xl border border-gray-200 focus:ring-1 focus:ring-brand focus:border-brand bg-gray-50/50"></textarea>
                            </div>
                            <div class="space-y-2">
                                <label class="text-sm font-bold text-gray-700">City</label>
                                <select class="w-full px-5 py-3 rounded-2xl border border-gray-200 focus:ring-1 focus:ring-brand focus:border-brand bg-gray-50/50">
                                    <option>Dhaka</option>
                            ...
                            <div class="space-y-2">
                                ...
                                <input type="text" placeholder="1212" class="w-full px-5 py-3 rounded-2xl border border-gray-200 focus:ring-1 focus:ring-brand focus:border-brand bg-gray-50/50">
                            </div>
                        </div>
                    </form>
                </div>

                <div x-data="{ method: 'cod' }" class="bg-white rounded-3xl border border-gray-100 p-8 shadow-sm">
                    <h2 class="text-2xl font-black text-gray-900 mb-8 flex items-center">
                        <span class="w-8 h-8 rounded-lg bg-gray-900 text-white text-sm flex items-center justify-center mr-3">2</span>
                        Payment Method
                    </h2>
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <button @click="method = 'cod'" :class="method === 'cod' ? 'border-brand bg-brand/5' : 'border-gray-100'" 
                        ...
                        <button @click="method = 'bkash'" :class="method === 'bkash' ? 'border-[#E2136E] bg-[#E2136E]/5' : 'border-gray-100'" 
                        ...
                        <button @click="method = 'card'" :class="method === 'card' ? 'border-indigo-600 bg-indigo-600/5' : 'border-gray-100'" 
            ...
            <aside class="w-full lg:w-[400px] space-y-6">
                <div class="bg-gray-900 rounded-3xl p-8 text-white shadow-2xl relative overflow-hidden">
                    ...
                    <div class="space-y-4 mb-8">
                        ...
                        <div class="flex items-center space-x-4">
                            <div class="w-16 h-16 rounded-2xl bg-white/10 shrink-0 overflow-hidden">
                                <img src="{{ Str::startsWith($item->thumbnail, 'http') ? $item->thumbnail : asset('uploads/product/' . $item->thumbnail) }}" class="w-full h-full object-cover">
                            </div>
                            <div class="flex-1">
                                <h4 class="text-xs font-bold line-clamp-1 truncate">{{ $item->name }}</h4>
                                <p class="text-[10px] text-gray-400">{{ $item->category->name }} x 1</p>
                            </div>
                            <span class="text-sm font-black">${{ $item->price }}</span>
                        </div>
                        @endforeach
                    </div>
                    ...
                    <button class="w-full bg-brand text-white py-4 rounded-2xl font-black text-sm mt-8 shadow-xl hover:scale-[1.02] transition transform active:scale-95 shadow-brand/20">
                        Complete Order Now
                    </button>
                </div>
                ...
                <div class="bg-white rounded-3xl border border-gray-100 p-6 shadow-sm">
                    <div class="flex space-x-2">
                        ...
                        <button class="px-6 py-2 bg-gray-100 text-gray-900 rounded-xl text-xs font-bold hover:bg-gray-200 transition">Apply</button>
                    </div>
                </div>
