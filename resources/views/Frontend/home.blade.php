        ...
        <div class="container mx-auto px-4 max-w-6xl">
            ...
            <div class="text-center mb-12">
                ...
                <p class="text-slate-500 text-lg max-w-xl mx-auto">
                    Track your vital metrics and nutritional requirements with our professional calculators.
                </p>
            </div>
            ...
            <div class="flex justify-center mb-10">
                <div class="bg-slate-200/50 p-1 rounded-xl flex items-center border border-slate-200">
                    <button @click="activeTab='weight'" 
                            :class="activeTab==='weight' ? 'bg-white shadow text-slate-800' : 'text-slate-500 hover:bg-slate-200/50'"
                            class="px-8 py-2.5 rounded-lg text-sm font-bold transition-all duration-300">
                        Weight Range
                    </button>
                    <button @click="activeTab='calorie'" 
                            :class="activeTab==='calorie' ? 'bg-white shadow text-slate-800' : 'text-slate-500 hover:bg-slate-200/50'"
                            class="px-8 py-2.5 rounded-lg text-sm font-bold transition-all duration-300">
                        Calorie Calculator
                    </button>
                </div>
            ...
            <div class="bg-white border border-slate-200 rounded-3xl shadow-sm overflow-hidden min-h-[450px]">
                            ...
                            <div class="max-w-2xl mx-auto">
                                ...
                                <div class="flex justify-center gap-4 mb-8">
                                    <button @click="chartGender='male'; lookupResult=null" 
                                            :class="chartGender==='male' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-slate-600 border-slate-200 hover:border-blue-300'"
                                    ...
                                    <button @click="chartGender='female'; lookupResult=null" 
                                            :class="chartGender==='female' ? 'bg-pink-600 text-white border-pink-600' : 'bg-white text-slate-600 border-slate-200 hover:border-pink-300'"
                                            class="flex-1 border-2 py-4 rounded-xl font-bold text-sm tracking-wide transition-all shadow-sm flex items-center justify-center gap-2">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                                        Female
                                    </button>
                                ...
                                <div class="flex flex-col md:flex-row items-center gap-3">
                                    <div class="relative flex-1 w-full">
                                        <input type="text" x-model="lookupHeight" @keyup.enter="findIdealWeight()" placeholder="Enter Height (e.g. 170 or 4.6)"
                                ...
                                <p class="text-center mt-4 text-xs text-slate-400 font-medium tracking-wide">
                                    Use <span class="text-slate-600 font-bold">CM</span> (170) or <span class="text-slate-600 font-bold">Feet.Inch</span> (4.6)
                                </p>
                            </div>
                        </div>

                        <!-- Result Section (Clean Cards) -->
                        <div x-show="lookupResult" class="lg:col-span-12 grid grid-cols-1 md:grid-cols-2 gap-6 mt-6 border-t border-slate-100 pt-10" x-transition:enter="transition-opacity duration-500">
                            <div class="bg-slate-50 p-8 rounded-2xl border border-slate-100 relative group">
                                <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-4 block">Optimal weight range</span>
                                <div class="space-y-4">
                                    <div class="flex items-baseline gap-2">
                                        <span class="text-5xl font-bold text-brand" x-text="lookupResult ? lookupResult.weightKg : ''"></span>
                                        <span class="text-sm font-bold text-slate-400 uppercase">KG</span>
                                    </div>
                                    <div class="w-full h-px bg-slate-200"></div>
                                    <div class="flex items-baseline gap-2">
                                        <span class="text-2xl font-bold text-slate-700" x-text="lookupResult ? lookupResult.weightLb : ''"></span>
                                        <span class="text-xs font-bold text-slate-400 uppercase tracking-tighter">LB (POUNDS)</span>
                                    </div>
                                </div>
                            </div>
                            <div class="bg-white border border-slate-200 p-8 rounded-2xl shadow-sm">
                                <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-4 block">Calculated Metric</span>
                                <div class="flex items-center gap-4">
                                    ...
                                    <span class="text-3xl font-bold text-slate-800" x-text="lookupResult ? lookupResult.h : ''"></span>
                                </div>
                            </div>
                        </div>
                ...
                <div x-show="activeTab==='calorie'" style="display:none" class="p-8 md:p-12" x-transition:enter="transition-opacity duration-300">
                    <div class="max-w-4xl mx-auto">
                        <!-- Calorie Gender Selection (Matching Weight Lab Style) -->
                        <div class="grid grid-cols-2 gap-4 mb-10">
                            <button @click="gender='male'; resetCalorie()" 
                                    :class="gender==='male' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-slate-600 border-slate-200 hover:border-blue-300'"
                                    class="flex-1 border-2 py-4 rounded-xl font-bold text-sm tracking-wide transition-all shadow-sm flex items-center justify-center gap-2">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                                Male
                            </button>
                            <button @click="gender='female'; resetCalorie()" 
                                    :class="gender==='female' ? 'bg-pink-600 text-white border-pink-600' : 'bg-white text-slate-600 border-slate-200 hover:border-pink-300'"
                                    class="flex-1 border-2 py-4 rounded-xl font-bold text-sm tracking-wide transition-all shadow-sm flex items-center justify-center gap-2">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                                Female
                            </button>
                        ...
                        <div x-show="result" class="mt-12 grid grid-cols-1 md:grid-cols-12 gap-6" x-transition:enter="transition-all duration-300">
                            <div class="md:col-span-12 bg-slate-900 px-10 py-10 rounded-2xl flex flex-col md:flex-row items-center justify-between gap-8">
                                <div>
                                    <h5 class="text-brand font-bold uppercase text-[10px] tracking-widest mb-3">Maintenance Calories</h5>
                                    <div class="flex items-baseline gap-3">
                                        <span class="text-5xl md:text-6xl font-bold text-white leading-none tracking-tight" x-text="result ? result.moderate : '0'"></span>
                                        <span class="text-lg font-bold text-slate-500 uppercase">kcal/day</span>
                                    </div>
                                </div>
                                <p class="text-slate-400 text-xs font-medium max-w-[200px] text-center md:text-right italic">
                                    Calculated for moderate physical activity (3-5 workouts/week).
                                </p>
                            </div>
                            <div class="md:col-span-6 bg-slate-50 p-8 rounded-2xl border border-slate-100 flex items-center justify-between">
                                <span class="text-xs font-bold text-slate-400 uppercase tracking-widest">Basal Metabolic Rate</span>
                                <span class="text-2xl font-bold text-slate-800" x-text="result?.sedentary + ' KCAL'"></span>
                            </div>
                            <div class="md:col-span-6 bg-slate-50 p-8 rounded-2xl border border-slate-100 flex items-center justify-between">
                                <span class="text-xs font-bold text-slate-400 uppercase tracking-widest">Athletic (Highly Active)</span>
