...
<div class="p-6 max-w-4xl">
    <div class="mb-6 flex items-center space-x-4">
        ...
        <h1 class="text-2xl font-black text-heading">Edit FAQ</h1>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-default p-10">
        <form action="{{ route('faq.update', $faq->id) }}" method="POST" class="space-y-8">
            @csrf @method('PUT')
            <div class="space-y-2">
                <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Question *</label>
                <input type="text" name="question" required value="{{ old('question', $faq->question) }}"
            ...
            <div class="space-y-2">
                ...
                <textarea name="answer" rows="6" required
                    class="w-full bg-gray-50 border border-default rounded-xl px-6 py-3.5 text-sm font-bold focus:outline-none focus:ring-1 focus:ring-brand focus:border-brand">{{ old('answer', $faq->answer) }}</textarea>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-2">
                    <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Order Number</label>
                    <input type="number" name="order_num" value="{{ old('order_num', $faq->order_num) }}"
                        class="w-full bg-gray-50 border border-default rounded-xl px-6 py-3.5 text-sm font-bold focus:outline-none focus:ring-1 focus:ring-brand focus:border-brand">
                </div>
                <div class="space-y-2">
