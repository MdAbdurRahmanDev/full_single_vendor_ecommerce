    ...
    <div class="p-6 space-y-8 w-full">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('admin.order.index') }}"
                class="text-xs font-bold text-gray-400 hover:text-brand uppercase tracking-widest flex items-center">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-default p-8 space-y-6">
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex flex-col">
                        <span class="text-[10px] font-black uppercase tracking-widest text-gray-400">Shipping
                            Destination</span>
                        <span class="font-bold text-gray-900 leading-relaxed">{{ $order->address }}, {{ $order->city }}
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-default p-8 space-y-6">
                ...
                <a href="{{ route('admin.order.invoice', $order->id) }}" target="_blank"
                    class="w-full bg-brand/5 text-brand border border-brand/20 py-4 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-brand hover:text-white transition-all shadow-sm flex items-center justify-center space-x-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M9 12h6m-6 4h12M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                    </svg>
                ...
                <form action="{{ route('admin.order.status', $order->id) }}" method="POST" class="space-y-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="order_status"
                            class="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 text-sm font-bold text-gray-900 focus:ring-1 focus:ring-brand focus:border-brand outline-none transition uppercase">
                    ...
                    <button type="submit"
                        class="w-full bg-gray-900 text-white py-4 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-brand transition-all shadow-xl active:scale-95 transform">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-default p-8">
                ...
                @foreach ($order->items as $item)
                    <div class="flex items-center justify-between pb-6 border-b border-default last:border-0 last:pb-0">
                        <div class="flex items-center space-x-4">
                            <img src="{{ filter_var($item->product->thumbnail, FILTER_VALIDATE_URL) ? $item->product->thumbnail : asset('uploads/products/' . $item->product->thumbnail) }}"
                                class="w-12 h-12 rounded-lg object-cover shadow-sm bg-gray-100"
                                alt="{{ $item->product->name }}">
                            <div>
                                <p class="text-sm font-black text-gray-900 tracking-tight lowercase">
                                    {{ $item->product->name }}</p>
                                <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Qty:
            ...
            <div class="mt-10 pt-10 border-t border-default space-y-4">
                ...
                <div class="flex justify-between items-center text-xl">
                    ...
                    <span
                        class="text-3xl font-black text-brand tracking-tighter">৳{{ number_format($order->total_amount, 2) }}</span>
                </div>
            </div>
        </div>
    </div>
